...
public interface ProductRepository extends JpaRepository<Product, Long> {
    ...
    @Query(value = "SELECT * FROM shopcart.products p WHERE p.is_deleted = false AND p.is_activated = true ORDER BY RANDOM() LIMIT 20", nativeQuery = true)
    List<Product> listViewProducts();


    @Query(value = "select * from shopcart.products p inner join shopcart.categories c on c.category_id = p.category_id where p.category_id = ?1 ", nativeQuery = true)
